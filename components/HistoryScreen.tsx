...
function ratingColor(r: number) {
  if (r >= 8) return 'text-emerald-600 border-emerald-300 bg-emerald-50'
  if (r >= 5) return 'text-amber-600 border-amber-300 bg-amber-50'
    ...
    <div className="min-h-screen bg-white text-gray-900 px-6 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            <h1 className="text-3xl font-black tracking-tighter text-gray-900">Pitch History</h1>
            {!loading && (
          ...
          <button
            onClick={onBack}
            className="text-xs border border-gray-200 hover:border-orange-400 text-gray-500 hover:text-gray-800 px-3 py-1.5 rounded-lg transition-all"
          >
            ← Back
          </button>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-gray-400 text-sm text-center py-20">No pitches yet. Go pitch something!</p>
        ) : (
          <div className="space-y-3">
            {pitches.map(pitch => {
              ...
              return (
                <div key={pitch.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                  <button
                    className="w-full text-left px-5 py-4 flex items-center gap-4 hover:bg-orange-50 transition-colors"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-gray-900 truncate">{pitch.product}</p>
                      <p className="text-gray-400 text-xs mt-0.5">{formatDate(pitch)}</p>
                    </div>
                    <span className="text-gray-600 text-sm font-mono shrink-0">
                      {formatCapital(pitch.capitalCommitted)}
                    </span>
                    {delta !== null && (
                      <span className={`text-xs font-semibold shrink-0 ${delta >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
